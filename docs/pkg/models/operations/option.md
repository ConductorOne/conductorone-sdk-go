...
```go
conductoronesdkgo.WithServerURL("https://api.example.com")
```
...
```go
conductoronesdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
conductoronesdkgo.WithServerIndex(1)
```
...
```go
conductoronesdkgo.WithTenantDomain(/* ... */)
```
...
```go
conductoronesdkgo.WithClient(httpClient)
```
...
```go
conductoronesdkgo.WithSecuritySource(/* ... */)
```
...
```go
conductoronesdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
conductoronesdkgo.WithTimeout(30 * time.Second)
```
